...
/**
 * ¿Entra en el período actual de Caja (sesión abierta)?
 * - Pagos y gastos normales: instante > último cierre.
 * - Sueldos (gasto con profesorId): nunca — descuentan del total de caja pero no mezclan el resumen de la sesión abierta.
 */
export function estaEnPeriodoAbiertoCaja(mov: Pago | Gasto, ultimoCierre: CierreCaja | null): boolean {
  const g = mov as Gasto;
  if ('profesorId' in g && g.profesorId) return false;
...
export function buildCierreRetiro(
  ...
  const inWinGasto = (g: Gasto) => {
    if (g.profesorId) return false; // sueldos no entran en el snapshot del cierre (operativo)
