                ...
                {historialAsistenciasLoading ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-500 mb-2">
                      Mismo formato que el calendario de clases · Verde = asistió, Rojo = no asistió
                    </p>
                    <div className="overflow-x-auto -mx-2 sm:mx-0">
                      <div className="min-w-[520px]">
                        {(() => {
                          ...
                          const getEstadoAsistencia = (turnoId: string): 'asistio' | 'no_asistio' | null => {
                            const a = asisDeSemana.find((x) => x.turnoId === turnoId);
                          ...
                          const renderCelda = (dia: number, hora: string) => {
                            ...
                            const enTurno = turno?.alumnoIds.includes(alumnoHistorial?.id ?? '');
                            const estado = turno ? getEstadoAsistencia(turno.id) : null;
                            if (!turno || (!enTurno && !estado)) return <div key={`${dia}-${hora}`} className="p-2 min-h-[48px] border-r border-gray-200 last:border-r-0 bg-gray-50" />;
                            return (
                              <div
                                key={`${dia}-${hora}`}
                                className="p-2 min-h-[48px] border-r border-gray-200 last:border-r-0 flex items-center justify-center bg-white"
                              >
                                {estado === 'asistio' ? (
                                  <span className="p-1.5 rounded bg-green-600 text-white" title={`${turno.titulo || 'Clase'} - Asistió`}>
                                    <Check className="w-4 h-4" />
                                  </span>
                                ) : estado === 'no_asistio' ? (
                                  <span className="p-1.5 rounded bg-red-600 text-white" title={`${turno.titulo || 'Clase'} - No asistió`}>
                              ...
                              <div className="border-x border-b border-gray-200">
                                ...
                                {horariosMananaHistorial.map((hora) => (
                                  <div key={hora} className="grid grid-cols-8 border-b border-gray-200 last:border-b-0">
                                    ...
                                    {diasSemana.map((dia) => renderCelda(dia, hora))}
                                  </div>
                                ))}
                              </div>
                    ...
                    {historialAsistencias.length > 0 && (
                      <details className="mt-3">
                        <summary className="text-xs font-medium text-gray-600 cursor-pointer hover:text-gray-800">Ver lista</summary>
