...
const Alumnos = () => {
  ...
  const handleDelete = async (id: string) => {
    const ok = await toast.confirm(
      '¿Desactivar alumno? Se saca de todas las clases del calendario (el cupo queda libre). Pagos e historial se conservan. Si lo reactivás, hay que anotarlo de nuevo.',
      {
    ...
    try {
      ...
      await loadAlumnos();
      toast.success('Alumno desactivado y sacado del calendario.');
    } catch (error) {
  ...
  const handleActivar = async (id: string) => {
    try {
      ...
      await loadAlumnos();
      toast.success('Alumno reactivado. Anotalo de nuevo en el calendario.');
    } catch (error) {
                              ...
                              <ul className="space-y-1 text-xs">
                                {items.map((a) => (
                                  <li key={a.id} className={`flex items-center gap-2 ${(a.estado ?? 'asistio') === 'asistio' ? 'text-green-700' : 'text-red-700'}`}>
                                    ...
                                    <span className="shrink-0 text-[10px] font-medium">{(a.estado ?? 'asistio') === 'asistio' ? 'Asistió' : 'No asistió'}</span>
                                    <button
                                      type="button"
